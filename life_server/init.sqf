...
if(isNil {GVAR_UINS "life_sql_id"}) then {
	...
	try {
		diag_log format["9:ADD_DATABASE:%1",EXTDB_SETTING(getText,"DatabaseName")];
		_result = EXTDB format["9:ADD_DATABASE:%1",EXTDB_SETTING(getText,"DatabaseName")];
		if(!(EQUAL(_result,"[1]"))) then {throw "extDB2: Error with Database Connection1"};
		...
		if(!(EQUAL(_result,"[1]"))) then {throw "extDB2: Error with Database Connection2"};
	} catch {
		diag_log _exception;
...
["CALL decrementContainerDuration",1] call DB_fnc_asyncCall;
